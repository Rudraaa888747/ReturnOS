...
export interface Order {
  ...
  createdAt: string
  items: OrderItem[]
  /** Latest still-open return for this order, if any. Reference + status only. */
  activeReturn: { id: string; returnNumber: string; status: ReturnStatus } | null
}
...
/** Customer-safe inspection projection: no internal notes, no staff identity. */
...
/** Customer-safe disposition projection: outcome only, no economics or staff identity. */
...
/** Authoritative per-item remaining-returnable projection from the backend. */
...
/** Authoritative order-level return-eligibility projection from the backend. */
