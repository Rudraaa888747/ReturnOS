...
@RestController @RequestMapping("/api/v1") public class DocsController {
  ...
  private final JdbcTemplate jdbc; private final Path uploadDir;
  DocsController(JdbcTemplate jdbc,@Value("${returnos.upload-dir:${UPLOAD_DIR:./uploads}}") String dir) throws java.io.IOException {this.jdbc=jdbc;this.uploadDir=Path.of(dir);Files.createDirectories(uploadDir);}
  ...
  @GetMapping("/documents/{id}/download") ResponseEntity<byte[]> download(@AuthenticationPrincipal CurrentUser u,@PathVariable String id) throws java.io.IOException {customer(u);
    ...
    var doc=rows.getFirst();Path target=Path.of((String)doc.get("storage_path"));
    if(!Files.exists(target))throw new ApiException(HttpStatus.NOT_FOUND,"FILE_MISSING","Stored file is no longer available");
    return ResponseEntity.ok().contentType(MediaType.parseMediaType((String)doc.get("mime"))).body(Files.readAllBytes(target));}
  ...
  @PostMapping("/uploads/return/{returnId}") @ResponseStatus(HttpStatus.CREATED) @Transactional Map<String,Object> upload(@AuthenticationPrincipal CurrentUser u,@PathVariable String returnId,@RequestParam(value="file",required=false) MultipartFile file,@RequestParam(value="kind",required=false) String kind) throws java.io.IOException {customer(u);
    ...
    String storedName=System.currentTimeMillis()+"-"+UUID.randomUUID()+ALLOWED_MIME.get(mime);
    Path target=uploadDir.resolve(storedName);file.transferTo(target);
    ...
    jdbc.update("insert into documents(id,return_id,user_id,kind,filename,mime,size,storage_path,created_at) values(?,?,?,?,?,?,?,?,?)",id,ret.get("id"),u.id(),k,original,mime,file.getSize(),target.toString(),t);
