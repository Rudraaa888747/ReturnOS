-- Demo baseline mirroring backend/src/seed.ts (catalogue, categories, demo
-- accounts, seeded orders + return). Idempotent: safe to apply over any
-- database. Stock is never restocked here; inventory buckets are only
...
-- Demo accounts (provisioned, never via signup; known passwords are the
-- same demo credentials as the TypeScript seed) -----------------------------
INSERT INTO users(id,email,password_hash,full_name,role,warehouse_id,active,created_at) VALUES
 ('u-demo-rudra','rudrachokshi441@gmail.com','$2a$10$N6UowyubFgTYvoIDvCbB1.snTMqB/zwdp6Q3xDs.vrcIFOwcAUsSu','Rudra Chokshi','CUSTOMER',NULL,true,now()-interval '60 days'),
...
INSERT INTO customer_profiles(user_id,phone,comm_prefs,notif_prefs) VALUES
 ('u-demo-rudra','+919876543210','{"email":true,"sms":false}'::jsonb,'{"returns":true,"offers":false}'::jsonb),
...
INSERT INTO addresses(id,user_id,label,full_name,line1,line2,city,state,postal_code,country,phone,is_default) VALUES
 ('addr-u-demo-rudra','u-demo-rudra','HOME','Rudra Chokshi','221 MG Road','Apartment 4B','Bengaluru','Karnataka','560001','IN','+919876543210',true),
...
INSERT INTO orders(id,order_number,customer_id,status,subtotal_paise,shipping_paise,payment_status,payment_method,carrier,tracking_number,created_at,delivered_at) VALUES
 ('o-2026-1001','ORD-2026-1001','u-demo-rudra','DELIVERED',589700,9900,'PAID','TEST','ReturnOS Logistics','TRK1001AAA',now()-interval '11 days',now()-interval '6 days'),
 ('o-2026-1002','ORD-2026-1002','u-demo-rudra','DELIVERED',849900,9900,'PAID','TEST','ReturnOS Logistics','TRK1002AAA',now()-interval '25 days',now()-interval '20 days'),
 ('o-2026-1003','ORD-2026-1003','u-demo-rudra','DELIVERED',1299500,9900,'PAID','TEST','ReturnOS Logistics','TRK1003AAA',now()-interval '50 days',now()-interval '45 days')
ON CONFLICT(id) DO NOTHING;
...
INSERT INTO returns(id,return_number,order_id,customer_id,status,resolution_type,description,created_at,updated_at) VALUES
 ('r-2026-0841','RET-2026-0841','o-2026-1002','u-demo-rudra','REQUESTED','REFUND','Sole stitching came apart after first use.',now()-interval '2 days',now()-interval '2 days')
ON CONFLICT(id) DO NOTHING;
...
SELECT 'nt-0841','u-demo-rudra','r-2026-0841','RETURN_STATUS','Return request received','Return RET-2026-0841 was created and is awaiting review.',now()-interval '2 days'
WHERE NOT EXISTS (SELECT 1 FROM notifications WHERE user_id='u-demo-rudra' AND return_id='r-2026-0841' AND type='RETURN_STATUS');
